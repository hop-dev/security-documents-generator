...
const createLoginDoc = ({
  username = 'johndoe',
  hostname = 'riskyhost.com',
  ip = '1.1.1.1',
  sourceIp = '2.2.2.2',
  timestamp,
}: {
...
const createPrivilegeDoc = ({
  username = 'adminuser',
  targetUsername = 'mahopki',
            ...
            modified_properties: [
              {
                ...
                new_value:
                  'johndoe (S-1-5-21-123456789-987654321-543216789-1001)',
              },
            ],
          },
        ],
...
export const loginToCriticalAsset = async ({
  namespace = 'default',
  username,
  init,
  hostname = 'critical_host',
}: {
  username: string;
  namespace?: string;
  init?: boolean;
...
const multipleLoginsOverTime = async ({
  namespace = 'default',
  count = 10,
  username,
  init,
...
export const multipleLoginsFromDifferentIps = async ({
  namespace = 'default',
  count,
  username,
  init,
  ...
  await bulkCreateLogins([...userDocs, ...otherUserDocs], namespace);
};

export const privilegeEscalation = async ({
  namespace = 'default',
  username,
  init,
}: {
  ...
  const docs = [
    ...NON_PRIVILEGED_GROUPS.map((groupName, i) =>
      createPrivilegeDoc({
        targetUsername: username,
        groupName,
        timestamp: NON_PRIVLEGED_GROUP_TIMESTAMPS[i],
      }),
    ),
    ...PRIVILEGED_GROUPS.map((groupName) =>
      createPrivilegeDoc({ targetUsername: username, groupName }),
    ),
  ];
...
export const createSimilarUsers = async ({
  namespace = 'default',
  init,
}: {
  namespace?: string;
...
export const createPrivmonData = async ({
  namespace = 'default',
  loginsCount,
  username,
  init,
  ...
  await multipleLoginsFromDifferentIps({
    count: loginsCount,
    username,
    namespace,
  });
